...
/**
 * Renders a Mapbox map component with various layers and interactive features.
 */
export function FeminicidiosMap(properties: MapboxMapProps) {
	...
	useEffect(() => {
		...
		return () => {
			if (
				mapRef.current?.getLayer('area-sin-cubrimiento-de-sitio-layer')
			) {
	...
	useEffect(() => {
		...
		return () => {
			if (
				mapRef.current &&
				mapRef.current.getLayer('resago-social-layer')
			) {
	...
	useEffect(() => {
		...
		return () => {
			if (
				mapRef.current &&
				mapRef.current.getLayer('sitios-de-apoyo-layer')
			) {
