...
export default function InformationPanel({
	...
	useEffect(() => {
		if (showInformationPanel) {
			...
			fetch('/prueba.md')
				...
				.catch(error =>
					console.error(
						'Error al cargar el archivo Markdown:',
						error,
					),
				);
		} else {
	...
	const markdownComponents = {
		h1: (props: any) => (
			<h1 className={`${getHeadingClass(1)} mt-4`} {...props} />
		),
		h2: (props: any) => (
			<h2 className={`${getHeadingClass(2)} mt-4`} {...props} />
		),
		h3: (props: any) => (
			<h3 className={`${getHeadingClass(3)} mt-4`} {...props} />
		),
		h4: (props: any) => (
			<h4 className={`${getHeadingClass(4)} mt-4`} {...props} />
		),
		h5: (props: any) => (
			<h5 className={`${getHeadingClass(5)} mt-4`} {...props} />
		),
		h6: (props: any) => (
			<h6 className={`${getHeadingClass(6)} mt-4`} {...props} />
		),
		blockquote: (props: any) => (
			<blockquote className={getBlockquoteClass()} {...props} />
